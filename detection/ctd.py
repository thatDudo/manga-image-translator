...
class ComicTextDetector(CommonDetector):
    ...
    def _load(self, input_size=1024, half=False, nms_thresh=0.35, conf_thresh=0.4, mask_thresh=0.3, act='leaky'):
        model_path = self._get_model_path()
        if self._use_cuda:
            self.model = TextDetBase(model_path, device=self.device, act=act)
            self.model.cuda()
            self.backend = 'torch'
        else:
    ...
    def _detect(self, img, refine_mode=REFINEMASK_INPAINT, keep_undetected_mask=False, bgr2rgb=True):
        ...
        mask_refined = refine_mask(img, mask_raw, blk_list, refine_mode=refine_mode)
        if keep_undetected_mask:
            mask_refined = refine_undetected_mask(img, mask_raw, mask_refined, blk_list, refine_mode=refine_mode)
